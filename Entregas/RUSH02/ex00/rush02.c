...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/28 21:39:44 by jzsigmon          #+#    #+#             */
/*   Updated: 2019/07/28 21:41:14 by jzsigmon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		first_line_02(int x, char *tab)
{
	int counter;
...
void		last_line_02(int x, int y, char *tab)
{
	int counter;
...
void		middle_lines_02(int x, int y, char *tab)
{
	int counter;
...
char		*rush_02(int x, int y)
{
	...
	while (counter < y)
	{
		if (counter == 0)
			...
			last_line_02(x, y, tab);
		else
			middle_lines_02(x, counter, tab);
		ft_putchar('\n', tab, x + ((x + 1) * counter));
		counter++;
	}
	tab[((x + 1) * y)] = '\0';
